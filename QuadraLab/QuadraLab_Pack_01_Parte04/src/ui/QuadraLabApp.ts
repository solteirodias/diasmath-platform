...
export class QuadraLabApp {
  ...
  private renderLab(): void {
    ...
    const factorPanel = new FactorizationPanel((factorization) => {
      this.renderer?.setFactorization(factorization);
      this.toast?.show(
        factorization
          ? "Retângulo dos fatores exibido."
          : "Construção ocultada."
      );
    });
    const equation = new EquationPanel(
      this.board,
      (label, coefficients) => {
        status.setEquation(label);
        factorPanel.analyze(coefficients);
        this.renderer?.setFactorization(null);
        this.toast?.show("Peças geradas com sucesso.");
      }
    );
    const cases = new CaseLibrary((example) => equation.generateFrom(example));
    const inspector = new Inspector(this.board);

    leftColumn.append(equation.element, cases.element, palette.element);
    ...
    rightColumn.append(factorPanel.element, inspector.element);
    ...
    if (this.board.allTiles.length === 0) {
      ...
      factorPanel.analyze({ a: 1, b: 5, c: 6 });
    }
  }
